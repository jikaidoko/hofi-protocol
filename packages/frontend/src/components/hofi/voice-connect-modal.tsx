...
import { Mic, Mail, Lock, User as UserIcon, Loader2 } from "lucide-react";
...
// Subdivisión del modo email: login para usuarios existentes, signup para
// crear cuenta nueva. La tabla `users` es la fuente de verdad (Cloud SQL).
...
export function VoiceConnectModal({
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    const url = emailMode === "signup" ? "/api/auth/register" : "/api/auth/login";
    const payload =
      emailMode === "signup"
        ? { email, password, memberName }
        : { email, password };
    ...
    try {
      ...
      const fallbackMsg =
        emailMode === "signup"
          ? "Registration failed. Try a different email or name."
  ...
  // ─── Social / OAuth ────────────────────────────────────────────────────────
  // Google / Telegram / X aún NO están implementados — los botones quedan
  // visibles pero deshabilitados con un tooltip "Coming soon" para que el
  // usuario vea que están planificados. El handler queda como placeholder.

  const handleSocialLogin = (_provider: "google" | "telegram" | "x") => {
    setError("Social login coming soon — please use voice or email for now");
  };
        ...
        <div className="space-y-6 py-4">
          ...
          {authMethod === "traditional" && (
            <div className="space-y-4">
              ...
              <div className="flex rounded-md bg-muted/20 p-0.5 text-xs">
                <button
                  type="button"
                  onClick={() => { setEmailMode("login"); setError(null); }}
                  className={cn(
                    "flex-1 py-1.5 px-2 rounded-sm font-medium transition-all",
                    emailMode === "login"
                      ? "bg-card shadow-sm text-foreground"
                      : "text-muted-foreground hover:text-foreground"
                  )}
                ...
                <Button type="submit" className="w-full" disabled={isSubmitting}>
                  {isSubmitting ? (
                    <>
                      ...
                      {emailMode === "signup" ? "Creating account…" : "Connecting…"}
                    </>
                  ) : emailMode === "signup" ? (
                    "Create account"
                  ) : (
              ...
              {/* Social Login — los proveedores OAuth aún no están conectados.
                  Los botones quedan visibles pero deshabilitados con un
                  tooltip "Coming soon" para señalizar que están planificados. */}
              <div className="grid grid-cols-3 gap-3">
                ...
                <Button
                  ...
                  className="h-11 bg-white hover:bg-gray-50 border-gray-300 text-gray-700 opacity-60 cursor-not-allowed"
                >
                ...
                <Button
                  ...
                  disabled
                  title="Telegram sign-in coming soon"
                  aria-label="Telegram sign-in (coming soon)"
                  className="h-11 bg-[#0088cc] hover:bg-[#0077b5] border-[#0088cc] text-white opacity-60 cursor-not-allowed"
                >
                ...
                <Button
                  ...
                  aria-label="X sign-in (coming soon)"
                  className="h-11 bg-black hover:bg-gray-900 border-black text-white opacity-60 cursor-not-allowed"
                >
                  <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
              ...
              <p className="text-xs text-center text-muted-foreground">
                Social sign-in (Google, Telegram, X) coming soon. Voice and email
