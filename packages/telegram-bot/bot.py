...
def parsear_registro(texto: str) -> tuple[str, str] | tuple[None, None]:
    ...
    # PatrÃ³n: "soy X, holÃ³n/olÃ³n Y" - lazy match en nombre, permite espacios en holÃ³n
...
async def manejar_voz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🎙️ Procesando tu audio...")
    ...
    # â"€â"€ TODO mensaje de voz verifica identidad - permite compartir dispositivo â"€
...
async def _flujo_autenticacion(update, user_id, sesion, embedding, texto):
    ...
    Permite que una misma cuenta de Telegram sea compartida por la familia -
    ...
    if resultado:
        ...
        if palabras >= 6 and _es_descripcion_tarea(texto):
            # El audio describe una tarea directamente - auth + tarea en un mensaje
...
async def _flujo_registro_voz_2(update, user_id, sesion, embedding):
    ...
    await update.message.reply_text(
        f"Listo, {nombre}! 🎙️ Tu voz quedó registrada en *{holon}*\n"
        "Registré 2 muestras para mayor precisión. Los audios fueron descartados.\n\n"
        "Contame tu primera tarea. ¿Qué hiciste y cuántas horas le dedicaste?",
        parse_mode="Markdown"
    )
...
def _es_descripcion_tarea(texto: str) -> bool:
    ...
    # Patrones de presentaciÃ³n / saludo - NOT tareas
...
async def _flujo_tarea(update, user_id, sesion, texto):
    ...
    if resultado["aprobada"]:
        ...
        gnh       = metricas["gnh_score"]
        horas_acc = metricas["horas_total"]
        co2_tarea = round(resultado.get("carbono_kg") or hoca / 40.0, 1)
        conf_pct  = int(resultado.get("confianza", 0.8) * 100)
        cat       = resultado.get("categoria", "cuidado").replace("_", " ").title()
        ...
        await update.message.reply_text(
            ...
            f"_{razon}_\n\n"
            f"📊 *Tu impacto acumulado:*\n"
            f"💰 Balance: {balance} HoCa\n"
            f"🌱 CO₂ total: {co2_acc + co2_tarea} kg\n"
            f"😊 Índice de felicidad: {gnh} pts\n"
            f"🤝 Horas de cuidado: {horas_acc} hrs\n\n"
            "¿Tenés otra tarea para reportar? 🙏",
            parse_mode="Markdown"
        )
    else:
        conf_pct = int(resultado.get("confianza", 0) * 100)
        razon = resultado.get("razonamiento", "")
        await update.message.reply_text(
            f"🤔 *Esta tarea no pudo ser aprobada* ({conf_pct}%)\n\n"
            f"_{razon}_\n\n"
            "📨 La enviamos a los miembros del holón para evaluación colectiva.\n\n"
            "Podés intentarlo de nuevo con más detalle. 🙏",
            parse_mode="Markdown"
        )
...
def _parsear_tarea(texto: str) -> dict:
    ...
    HeurÃ­stica simple - mejorar con NLP o Gemini en iteraciÃ³n siguiente.
    """
    import re
